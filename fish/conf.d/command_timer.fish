# Thanks to https://github.com/jichu4n/fish-command-timer, modified a bit
...
if not set -q fish_command_timer_time_format
  set fish_command_timer_time_format '%d, %H:%M'
end
...
function fish_command_timer_postexec -e fish_postexec
  ...
  if begin
    ...
    set timing_str_colored (set_color -d $fish_command_timer_color)"$timing_str"(set_color normal)
  else
